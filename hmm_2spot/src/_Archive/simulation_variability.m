addpath('./utilities');
% Script to Test Expected Variability in Experimental Observations
%--------------------Define Simulation Parameters-------------------------%
%Set write paths

% subfolder = '3state_nn_cont_section5_1';
subfolder = '2state_noise_comparison';
outpath = ['../figs/fano_figs_number_fluctuations/' subfolder];
if exist([outpath]) ~= 7
    mkdir(outpath);
end
out_name = 'nn';
...
for j = 1:length(k_off_vec)
    ...
    noise = 0; %.1*w*r_emission(2)*deltaT*k_on_vec(j)/(k_on_vec(j)+k_off_vec(j));
    %-----------------------Run Simulation------------------------------------%
    ...
    %Iterate
    on_steps = [];
    for i = 1:n_traces*n_simulations
    ...
    var_vec_p = zeros(n_simulations, seq_length);
    var_vec_c = zeros(n_simulations, seq_length);
    mean_vec_p = zeros(n_simulations, seq_length);
    mean_vec_c = zeros(n_simulations, seq_length);
    fano_vec_p = zeros(n_simulations, seq_length);
    fano_vec_c = zeros(n_simulations, seq_length);
    for sim = 1:n_simulations
        var_vec_p(sim,:) = var(cm_trace_mat(:,(1+(sim-1)*n_traces):sim*n_traces)'/(fluo_per_rna*w))' ;
        var_vec_c(sim,:) = var(cm_trace_mat_cont(:,(1+(sim-1)*n_traces):sim*n_traces)'/(fluo_per_rna*w))' ;

        mean_vec_p(sim,:) = mean(cm_trace_mat(:,(1+(sim-1)*n_traces):sim*n_traces),2) / w / fluo_per_rna;
        mean_vec_c(sim,:) = mean(cm_trace_mat_cont(:,(1+(sim-1)*n_traces):sim*n_traces),2) / w / fluo_per_rna; 

        %%% Get  Fano %%%

        fano_vec_p(sim,:) = var_vec_p(sim,:) ./ mean_vec_p(sim,:);
        fano_vec_c(sim,:) = var_vec_c(sim,:) ./ mean_vec_c(sim,:);
    end
    ...
    subplot(1,3,1);
    hold on
    plot(mean_vec_p', 'Color', [cm(1,:) alph],'LineWidth',1.5);
    plot(mean_predicted,'LineStyle','--','Color', cm(30,:),'LineWidth',1.5);
    if plot_continuous
        plot(mean_predicted,'LineStyle','--','Color', cm(45,:),'LineWidth',1.5);
        plot(mean_vec_c','Color', [cm(45,:) alph],'LineWidth',1.5)
    end
    title(['Mean: Simulation-to-Simulation Variability (' num2str(K) 'States)'], 'fontsize',10);
    xlabel('Time Steps');

    subplot(1,3,2);
    hold on
    plot(var_vec_p', 'Color', [cm(1,:) alph],'LineWidth',1.5);
    plot(var_predicted_p,'LineStyle','--','Color', cm(30,:),'LineWidth',1.5);
    if plot_continuous
        plot(var_vec_c', 'Color', [cm(45,:) alph],'LineWidth',1.5);
        plot(var_predicted_c,'LineStyle','--','Color', cm(60,:),'LineWidth',1.5);
    end
    title(['Variance: Simulation-to-Simulation Variability (' num2str(K) ' States)'],  'fontsize',10);
    xlabel('Time Steps');

    subplot(1,3,3);
    hold on
    plot(fano_vec_p','Color', [cm(1,:) alph],'LineWidth',1.5);
    plot(1:length(fano_vec_p'),linspace(fano_predicted_p,fano_predicted_p,length(fano_vec_p)),'--','Color', cm(30,:),'LineWidth',1.5);
    if plot_continuous
        plot(fano_vec_c', 'Color', [cm(45,:) alph],'LineWidth',1.5);
    ...
    title(['Fano Factor: Simulation-to-Simulation Variability (' num2str(K) ' States)'],  'fontsize',10);
    if plot_continuous
