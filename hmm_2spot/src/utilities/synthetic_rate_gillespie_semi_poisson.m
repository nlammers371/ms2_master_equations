function gillespie = synthetic_rate_gillespie_semi_poisson(seq_length, alpha, ...
...
%             warning(['Fluo Per mRNA is being adjusted to: ' num2str(f_rna) ' to maintain consistency with continuous case']);
        ...
        for k = 1:seq_length
            ...
            arrival_events = arrival_times(1==(arrival_times > t_start).*(arrival_times <= t_end));
            
            for g = 1:(length(arrival_events))
                if isnan(fluo_MS2(k))
                    error('Start')
                end
                t = t_end - arrival_events(g);
                if t > alpha * deltaT
